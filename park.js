class ParkGenerator {
    constructor() {
        // The complete registry of all 63 US National Parks
        this.usParks = [
    ...
    pickRandom() {
        ...
        // Adds a little rustic flair while it "searches"
        display.innerHTML = "<em>Consulting the map...</em>";
        
        setTimeout(() => {
            display.innerHTML = `
                <div style="margin-top:20px; padding-top:15px; border-top: 2px solid #5C3A2E;">
                    <strong style="font-size: 1.8rem; color: #2D4033;">${park.name} National Park</strong><br>
                    <span style="font-size: 1.2rem; color: #8B5E3C;">EST. ${park.year} | ${park.state}</span>
                </div>`;
        }, 500);
    }
}

// Initialize the ranger so the button works
